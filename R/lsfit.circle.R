...
#    nlminb(start = init, obj = obj.fun, gradient = grad.fun, hessian=TRUE, x = x, y = y, ...)
    ...
    result$angles <- circular(atan2(y=result$y.centered, x=result$x.centered), type=type, units=units, template=template, modulo=modulo, rotation=rotation, zero=zero)
