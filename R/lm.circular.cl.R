...
lm.circular.cl <- function(y, x, init, verbose=FALSE, tol=1e-10) {
    ...
    if (missing(init)) stop("'init' is missing with no default")
    if (is.vector(x)) x <- cbind(x)
    if (NCOL(x)!=length(init)) stop("'init' must have the same number of elements as the columns of 'x'")
    y.orig <- y <- as.circular(y)
    ycircularp <- circularp(y)
    units <- ycircularp$units
